...
| `cwCreateDenom(tokenOwner: string, subdenom: string, paymentAmount: number, paymentDenom: string, gasPrice: string): Promise<DeliverTxResponse>` | Contract admin or owner | Create a new token denomination using the `minter` contract |
...
| `cwRevoke(collectionAddress: string, senderAddress: string, operator: string, gasPrice: string): Promise<DeliverTxResponse>` | Developer | Remove approval from NFT collection. For debugging |
